...
function normalizeADSBFlight(aircraft: any): MilitaryFlight {
    return {
        ...
        lat: Number(aircraft.lat ?? 0),
        lon: Number(aircraft.lon ?? 0),
        alt: aircraft.alt_baro === 'ground' ? 0 : Number(aircraft.alt_baro ?? aircraft.alt ?? 0),
        speed: Number(aircraft.gs ?? aircraft.speed ?? 0),
        track: Number(aircraft.track ?? aircraft.heading ?? 0),
